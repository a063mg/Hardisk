...
# items_color = 'Purple'
# items_size = 'Large'
items_name = u"Nylon Ditty Bags (Set of 3)"
items_color = 'Multi'
items_size = 'ASST'
...
customs_address = 'Kakoy to tam dom'
customs_city = 'Moscow'
								...
								if p.get('itemprop') == 'model': 
									if str(p.text) == items_color:
										...
										if pas:
											...
											elem = driver.find_elements_by_xpath("//input[@placeholder='CVV']")
											elem[0].send_keys(customs_CVV)
											...
											select = driver.find_elements_by_xpath("//select[@id='credit_card_month']")
											select = Select(select[0])
											select = select.select_by_visible_text(customs_date1)
											select = driver.find_elements_by_xpath("//select[@id='credit_card_year']")
											select = Select(select[0])
											pas = False
											finish = time.time()
											print(finish - start)
